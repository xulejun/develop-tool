...
public class RankRefreshJob {
    ...
    public void refreshHour() {
        ...
        for (int i = 1; i <= 26; i++) {
            redisTemplate.opsForZSet().incrementScore(HOUR_KEY + hour, String.valueOf((char) (96 + i)), RandomUtil.randomInt(10));
        }
    }
    ...
    private void refreshMonth(long hour) {
        ...
        redisTemplate.opsForZSet().unionAndStore(HOUR_KEY + hour, keyList, MONTH_KEY);
    ...
    private void refreshWeek(long hour) {
        ...
        // 把当前时间key，并且把后推24*7-1小时，共计24*7小时，求出并集放入week_key
        redisTemplate.opsForZSet().unionAndStore(HOUR_KEY + hour, keyList, WEEK_KEY);
    ...
    private void refreshDay(long hour) {
        ...
        redisTemplate.opsForZSet().unionAndStore(HOUR_KEY + hour, keyList, DAY_KEY);
